...
from grammar_induction import codelength_utils
from grammar_induction import derivation_utils
from grammar_induction import rule_utils
from grammar_induction import split_utils

from grammar_induction import qcfg_parser
...
InductionConfig = collections.namedtuple("InductionConfig", [
    ...
    "seed_exact_match",  # Temporarily, role as a seed_inner_most_phrases
    "balance_parens",
])
...
def induce_rules(examples, config):
    ...
    if config.seed_exact_match:
        exact_match_rules = exact_match_utils.get_exact_match_rules(examples_sample + examples_other)
        seed_rules |= exact_match_rules
    print("Seed rules after adding exact match rules for all examples: %s." %
          len(seed_rules))
